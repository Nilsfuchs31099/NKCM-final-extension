function [Zend,Mend] = get_Z(Z0,gZ0,GZ1,L0,LT,lg,gS0,SIG0,GS1,deltapb,tau0,theta2,pb,chi,omega,Dc,Et,M_1750,nu,mc,sigmaC,xi,M0,tf,gamma,alpha,sigmaL,phi_m,M_ref,psi_l,psi_m)
...
%   Detailed explanation goes here
    ...
    gZtrend(1) = gZ0;
    gLv = nan(T,1); 
    gLv(1) = (LT/L0)^lg-1; % initial population growth (climate term 0 at t = 0)
    gZ  = nan(T,1);
    gZ(1)  = gZtrend(1) + psi_l*gLv(1);
    ...
    tolx = 1e-6;
    for i = 2:T
        gZtrend(i) = (1-GZ1)*gZtrend(i-1);
        Z(i)       = Z(i-1)*(1+gZ(i-1));
	    L(i)       = L(i-1)^(1-lg)*LT^lg*(1-phi_m*max(M(i-1)-M_ref,0));	        % square the bracket for the quadratic version
        Lnc(i)  = Lnc(i-1)^(1-lg)*LT^lg;
        gLnc    = Lnc(i)/Lnc(i-1) - 1;
        gLv(i)     = L(i)/L(i-1) - 1;
        if i >= 3
            dM = max(M(i-1) - M(i-2), 0);
        else
            dM = 0;
        end                                                                     % 0 at the first step
        gZ(i)      = gZtrend(i) + psi_l*(gLv(i) - gLnc) - psi_m*dM;
