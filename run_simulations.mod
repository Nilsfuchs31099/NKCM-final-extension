...
% PLOT FIGURE 3
nx = 3;ny = 3;varnames=char('lny','pi100','rr100','lny_n','output_gap','rrn100','tau_USD','E','M');
...
green  = [ 50 168  82]/255;   % Paris-Agreement / baseline
red    = [194  68  43]/255;   % Laissez-faire
yellow = [224 182  11]/255;   % SCC / flexible-price
purple = [126  47 142]/255;   % carbon-budget rule
blue   = [ 33 114 176]/255;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% FIGURE 7. The social cost of carbon (in-sample, sticky vs flexible prices)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SCC   : social cost of carbon in the estimated (sticky-price) economy
% SCC_n : social cost of carbon in the natural (flexible-price) economy
% Both are expressed in current US$ per ton of CO2 (see eq. 30 of the paper).
T_hist     = dseries_to_num(endo_simul_ts)';
idx_SCC    = strmatch('SCC',  M_.endo_names,'exact');
idx_SCC_n  = strmatch('SCC_n',M_.endo_names,'exact');
scc_sticky = endo_simul_ts.data(:,idx_SCC);
scc_flex   = endo_simul_ts.data(:,idx_SCC_n);

figure('Name','Figure 7: The social cost of carbon');
hold on;
% shade NBER recessions (approximate, quarterly) within the plotting window
nber = [1990+2/4 1991+0/4; 2001+0/4 2001+3/4; 2007+3/4 2009+1/4; 2020+0/4 2020+1/4];
yl   = [10 45];
for ir = 1:size(nber,1)
    patch([nber(ir,1) nber(ir,2) nber(ir,2) nber(ir,1)],[yl(1) yl(1) yl(2) yl(2)],...
          [0.9 0.9 0.9],'EdgeColor','none','HandleVisibility','off');
end
plot(T_hist,scc_sticky,'-' ,'linewidth',2,'Color',green);
plot(T_hist,scc_flex  ,':','linewidth',2,'Color',yellow);
hold off; grid on; box on;
xlim([1985 2024]); ylim(yl);
ylabel('US$ / ton of CO_2');
title('Social cost of carbon','Interpreter','latex');
legend('Sticky-price economy','Natural (flexible-price) economy','Location','NorthWest');

idx_dmg   = strmatch('SCC',     M_.endo_names,'exact');
idx_mort  = strmatch('SCC_mort',M_.endo_names,'exact');
T_hist    = dseries_to_num(endo_simul_ts)';

scc_dmg   = endo_simul_ts.data(:,idx_dmg);
scc_mort  = endo_simul_ts.data(:,idx_mort);
scc_total = scc_dmg + scc_mort;

figure('Name','Figure 7b: SCC decomposition (productivity vs mortality)');
hold on;
plot(T_hist, scc_dmg  ,'--','linewidth',2,'Color',[50,96,168]/255);
plot(T_hist, scc_mort ,':' ,'linewidth',2,'Color',[194,68,43]/255);
plot(T_hist, scc_total,'-' ,'linewidth',2,'Color',[50,168,82]/255);
hold off; grid on; box on;
xlim([1985 2024]);
ylabel('US$ / ton of CO_2');
title('Social cost of carbon: productivity + mortality','Interpreter','latex');
legend('Productivity damage (SSV)','Mortality (Bressler-type)','Total','Location','NorthWest');
