...
@#if SIMULATIONS == 1
    ...
    SCC_mort    ${SCC^{m}_t}$   (long_name='SCC, mortality channel'), %new SCC
    SCC_mort_n  ${SCC^{m\star}_t}$ (long_name='SCC, mortality channel (natural)') %new SCC
...
phi_m       = 0;      % Baseline --> 0 to reproduce SSV exactly use 5.9e-8 for linear and 5.3e-11 for quadratic & upper bound 1.8e-7

/* Technology-growth extension */
psi_l       = 0;            % Jones semi-endogenous link; O reproduces SSV exactly 0.32-0.24 bound and upper bound 0.5
psi_m       = 0;            % direct climate effect; 0 reproduces SSV exactly, use 3e-5 and upper bound 2.4e-4

/* SCC extension */
VSL         = 7e6;               % value of a statistical life, real USD (EPA-style anchor).Key normative parameter -- run robustness over ~2e6-16e6 as in Bressler's VSLY range.
...
model;
	...
	@#if SIMULATIONS == 1
        ...
        [name='SCC, mortality channel (sticky)']
        SCC_mort   = beta*( (1+gZ)^(-sigmaC)*lb(+1)/lb   *SCC_mort(+1) + xi*phi_m*(L^(1-lg)*LT^lg)*VSL );
        [name='SCC, mortality channel (natural)']
        SCC_mort_n = beta*( (1+gZ)^(-sigmaC)*lb_n(+1)/lb_n*SCC_mort_n(+1) + xi*phi_m*(L^(1-lg)*LT^lg)*VSL );
	@#endif
...
steady_state_model;	
	...
	@#if SIMULATIONS == 1
        ...
        SCC_mort   = xi*phi_m*(L^(1-lg)*LT^lg)*VSL/(1/beta-1);
        SCC_mort_n = xi*phi_m*(L^(1-lg)*LT^lg)*VSL/(1/beta-1);
